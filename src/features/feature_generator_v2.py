...
FEATURE_MAP: Dict[str, str] = {
    ...
    # 전극 - 두께/압연
    "press1_thickness":"X_Anode_PV_Coating_W715F_N","press2_thickness":"X_Anode_PV_Coating_W00795F_N",
    "slitting_thickness":"X_Anode_PV_Coating_W001550_N","thk_cs_ds_1":"X_Anode_PV_Coating_D0009501_N",
    "roll_gap_1_ds":"X_Anode_PV_Coating_D0037110_F","rolling_force":"X_Anode_PV_Coating_D0003590_F",
    "main_roll_usage":"X_Anode_PV_Coating_R03680_F",
    # 전극 - 믹싱
    "solid_content":"X_Anode_PV_Coating_3_D0010964_N","solid_content_avg":"X_Anode_PV_Coating_3_D0010968_N",
    "mixer_motor_amp":"X_Anode_PV_Coating_2_D0012280_N","active_wt_sv":"X_Anode_PV_Coating_3_R01000_N",
    "active_wt_pv":"X_Anode_PV_Coating_R1006",
    # 전극 - 코팅
    "coat_line_speed":"X_Anode_PV_Coating_D0002500_N","dry_temp_mid_top":"X_Anode_PV_Coating_W002204_N",
    "supply_fan_rpm":"X_Anode_PV_Coating_D2720","coat_gap_sv":"X_Anode_PV_Coating_R13026",
    "coat_pump_rpm_pv":"X_Anode_PV_Coating_R13015","chamber_humidity":"X_Anode_PV_Coating_W220B",
    "ng_unfilled_wrinkle_top":"X_Anode_PV_Coating_R12304","ng_unfilled_wrinkle_back":"X_Anode_PV_Coating_R12319",
    # 전극 - 로딩/폭/정렬
    "loading_total":"X_Anode_PV_Coating_D2266","electrode_width":"X_Anode_PV_Coating_D0020800_N",
    "mismatch_top1":"X_Anode_PV_Coating_W001390_N","mismatch_top2":"X_Anode_PV_Coating_W001392_N",
    "anode_loading":"X_Anode_PV_Coating_ANODE_LOADING",  # placeholder
    "cathode_loading":"X_Anode_PV_Coating_CATHODE_LOADING",  # placeholder
    # 전극 - 슬리팅/재권취
    "slit_line_speed":"X_Anode_PV_Coating_R00332_N","slit_stretch_tension":"X_Anode_PV_Coating_D0006012_N",
    "slit_knife_usage_1":"X_Anode_PV_Coating_R24016_F","rewinder_length":"X_Anode_PV_Coating_D0006132_F",
    ...
    # 전극 LOT
    "electrode_lot":"X_Anode_PV_Coating_LOT_ID","anode_lot":"X_Anode_PV_Coating_ANODE_LOT",
    "cathode_lot":"X_Anode_PV_Coating_CATHODE_LOT",
    # 원자재/이물
    ...
    # 조립 - 6/17 인터뷰 추가 SV/PV 매핑 (순번 75~81)
    # 파일 2(SV 목록)에서 확인된 항목은 실제 X_SV 컬럼으로 반영, 미확인 항목은 placeholder 유지
    "cbd_down_time_s":"X_SV_Assembly_[2B] Chamber [공정#2] 하강시간",
    "cbd_up_time_s":"X_SV_Assembly_[2B] Chamber [공정#2] 상승시간",
    "ccr_l2":"X_SV_Assembly_CCR #2 전체 디스크 위치",
    "ccr_l2_spindle_pos":"X_SV_Assembly_CCR #2 스핀들 12 위치",
    "elf_pump_amount_sv":"X_SV_Assembly_[EL Filling Chamber] PUMP 주액량 설정값",
    "post_injection_wait_time":"X_SV_Assembly_주액 후 대기 시간",
    "cit_main_guide_vacuum":"X_SV_Assembly_CIT_MAIN_GUIDE_VACUUM",
    "upper_holder_rpm":"X_PV_Assembly_UPPER_HOLDER_RPM",
    "lower_holder_rpm":"X_PV_Assembly_LOWER_HOLDER_RPM",
    "vent_bending_angle":"X_PV_Assembly_VENT_BENDING_ANGLE",
    "vent_flange_angle":"X_PV_Assembly_VENT_FLANGE_ANGLE",
    "process_timestamp":"X_PV_Timestamp_PROCESS",
    "electrolyte_design_amount":"X_PV_Assembly_ELECTROLYTE_DESIGN_AMOUNT",
    "surface_nonmetal_foreign":"X_PV_Inspection_SURFACE_NONMETAL_FOREIGN",
    "loading_shortage":"X_Anode_PV_Coating_LOADING_SHORTAGE",
...
DEFAULT_CONSTS = {
    ...
    # 순번 75~81 추가 구현용 기준값
    "cbd_down_time_target_s": 0.0,
    "cbd_down_speed_threshold_proxy": 0.0,
    "ccr_l2_design": 0.0,
    "vent_bending_angle_target": 0.0,
    "vent_flange_angle_target": 0.0,
    "foreign_threshold": 1.0,
    "material_foreign_weight": 1.0,
    "process_foreign_weight": 1.2,
    "electrolyte_additive_target": 1.0,
    "cit_vacuum_target": 0.0,
    "design_change_timestamp": None,
}
...
class FeatureGenerator:
    ...
    def transform(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # 6/17 인터뷰 추가 항목 - 순번 75~81
        out = self._add_late_interview_features_75_81(data, out, ctx)
    ...
    def _get_raw_col(self, df: pd.DataFrame, key: str) -> pd.Series:
        """Get raw column by feature key without numeric coercion."""
        name = FEATURE_MAP.get(key, key)
        if name in df.columns:
            return df[name]
    ...
    def _calc_time_diff(self, df: pd.DataFrame, start_col: str, end_col: str) -> pd.Series:
        ...
        start = self._get_raw_col(df, start_col)
        end = self._get_raw_col(df, end_col)
        try:
    ...
    def _safe_div(self, numerator: pd.Series, denominator: pd.Series) -> pd.Series:
        """Safe division with zero denominator converted to NaN."""
        return numerator / denominator.replace(0, np.nan)

    def _row_sum(self, series_list: List[pd.Series]) -> pd.Series:
        """Row-wise sum preserving all-NaN rows as NaN."""
        if not series_list:
            return pd.Series(np.nan, index=[])
        return pd.concat(series_list, axis=1).sum(axis=1, min_count=1)

    def _row_max(self, series_list: List[pd.Series]) -> pd.Series:
        """Row-wise max preserving all-NaN rows as NaN."""
        if not series_list:
            return pd.Series(np.nan, index=[])
        return pd.concat(series_list, axis=1).max(axis=1, skipna=True)

    def _after_timestamp_flag(self, df: pd.DataFrame, ts_key: str, cutoff) -> pd.Series:
        """Flag rows after a design-change timestamp. Returns NaN if cutoff or timestamp is unavailable."""
        if cutoff is None:
            return pd.Series(pd.NA, index=df.index, dtype="Int8")
        ts = pd.to_datetime(self._get_raw_col(df, ts_key), errors="coerce")
        cutoff_ts = pd.to_datetime(cutoff, errors="coerce")
        if pd.isna(cutoff_ts):
            return pd.Series(pd.NA, index=df.index, dtype="Int8")
        return (ts >= cutoff_ts).astype("Int8")
    # =============================================================================
    ...
    def _add_late_interview_features_75_81(self, data: pd.DataFrame, out: pd.DataFrame, ctx: Dict) -> pd.DataFrame:
        """파일 3 기준 순번 75~81 미구현 항목 추가.

        구현 대상:
        - 75: CBD 비틀림 - 하강 안착 충분도, 상승-하강 정합
        - 76: CSZ TTS - CCR L2 반제품 치수 편차, Vent 각도×CCR L2 결합
        - 77: CSZ TTS - Vent 각도 데이터 가용 구간 플래그
        - 78: 저전압 - 이물 합산 임계 초과, 단일 출처 집중, 첨가제 보정 임계
        - 79: 저전압 - 원자재/공정 출처 가중 이물
        - 80: 용량/Energy - 전해액 발휘 가능 용량률, 3원인 동시 발현 지표
        - 81: CIT Taping - 메인가이드 진공압 적정구간 이탈
        """
        # 75. CBD 하강 거동
        # 파일 3 계산식은 하강속도 기준이나, 파일 2에서는 하강시간 SV만 확인됨.
        # 따라서 1 / 하강시간을 속도 프록시로 사용하여 방향성(값↑ = 양호)을 맞춘다.
        down_time = self._get_col(data, "cbd_down_time_s")
        up_time = self._get_col(data, "cbd_up_time_s")
        down_speed_proxy = self._safe_div(pd.Series(1.0, index=data.index), down_time)
        out["DV75__CBD 하강시간"] = down_time
        out["DV75__CBD 하강속도 프록시"] = down_speed_proxy
        out["DV75__CBD 하강 안착 충분도"] = down_speed_proxy - ctx.get("cbd_down_speed_threshold_proxy", 0.0)
        out["DV75__CBD 하강시간 초과 리스크"] = (down_time - ctx.get("cbd_down_time_target_s", 0.0)).clip(lower=0)
        out["DV75__CBD 상승-하강 정합"] = (up_time - down_time).abs()

        # 76. CSZ TTS: CCR L2 및 Vent 각도 결합
        ccr_l2 = self._get_col(data, "ccr_l2")
        ccr_l2_dev = (ccr_l2 - ctx.get("ccr_l2_design", 0.0)).abs()
        vent_bending = self._get_col(data, "vent_bending_angle")
        vent_flange = self._get_col(data, "vent_flange_angle")
        vent_bending_dev = (vent_bending - ctx.get("vent_bending_angle_target", 0.0)).abs()
        vent_flange_dev = (vent_flange - ctx.get("vent_flange_angle_target", 0.0)).abs()
        vent_angle_dev = self._row_sum([vent_bending_dev, vent_flange_dev])
        out["DV76__CCR L2 반제품 치수 편차"] = ccr_l2_dev
        out["DV76__Vent각도×CCR L2 결합"] = vent_angle_dev * ccr_l2_dev

        # 77. Vent 각도 측정 시작 이후 구간 플래그
        out["DV77__Vent 설계변경 이후 플래그"] = self._after_timestamp_flag(
            data, "process_timestamp", ctx.get("design_change_timestamp")
        )

        # 78~79. 저전압: 이물 임계/출처 가중
        material_foreign_cols = [
            self._get_col(data, "foreign_anode"),
            self._get_col(data, "foreign_cathode"),
            self._get_col(data, "foreign_conductor"),
            self._get_col(data, "foreign_electrolyte"),
            self._get_col(data, "foreign_can"),
        ]
        process_foreign_cols = [
            self._get_col(data, "foreign_hopper"),
            self._get_col(data, "foreign_winding"),
            self._get_col(data, "foreign_welding"),
        ]
        all_foreign_cols = material_foreign_cols + process_foreign_cols
        material_foreign_sum = self._row_sum(material_foreign_cols)
        process_foreign_sum = self._row_sum(process_foreign_cols)
        all_foreign_sum = self._row_sum(all_foreign_cols)
        max_single_foreign = self._row_max(all_foreign_cols)

        additive = self._get_col(data, "electrolyte_additive_low_voltage")
        additive_target = ctx.get("electrolyte_additive_target", 1.0) or np.nan
        additive_ratio = additive / additive_target
        adaptive_threshold = ctx.get("foreign_threshold", 1.0) * additive_ratio

        out["DV78__이물 합산 임계 초과"] = (all_foreign_sum - ctx.get("foreign_threshold", 1.0)).clip(lower=0)
        out["DV78__단일 출처 이물 집중"] = max_single_foreign
        out["DV78__첨가제 보정 이물 임계 초과"] = (all_foreign_sum - adaptive_threshold).clip(lower=0)
        out["DV79__출처 가중 이물"] = (
            ctx.get("material_foreign_weight", 1.0) * material_foreign_sum
            + ctx.get("process_foreign_weight", 1.2) * process_foreign_sum
        )

        # 80. 용량/Energy: 주입량 발휘율 및 3원인 교호
        actual_dose = self._get_col(data, "elf_final_dose_g")
        design_dose = self._get_col(data, "electrolyte_design_amount")
        sv_dose = self._get_col(data, "elf_pump_amount_sv")
        dose_reference = design_dose.where(design_dose.notna(), sv_dose)
        capacity_ratio = self._safe_div(actual_dose, dose_reference)
        injection_shortage = (1.0 - capacity_ratio).clip(lower=0)
        loading_shortage = self._get_col(data, "loading_shortage")
        surface_foreign = self._get_col(data, "surface_nonmetal_foreign")
        out["DV80__전해액 발휘 가능 용량률"] = capacity_ratio
        out["DV80__주입량부족×로딩부족×표면이물"] = injection_shortage * loading_shortage * surface_foreign

        # 81. CIT Taping: 메인가이드 진공압 U자 이탈
        cit_vacuum = self._get_col(data, "cit_main_guide_vacuum")
        out["DV81__CIT 메인가이드 진공압 이탈"] = (cit_vacuum - ctx.get("cit_vacuum_target", 0.0)).abs()
